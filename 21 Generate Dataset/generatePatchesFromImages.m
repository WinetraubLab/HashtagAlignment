...
if false % set to false to skip black patch
    im_bk = zeros(...
        patchSize_pix(1),... y
        patchSize_pix(2),... x
        3,'uint8');
    ...
    for y=1:patchOverlap_pix(1):(h-patchSize_pix(1))
        for x=x_Start:patchOverlap_pix(2):max([w-patchSize_pix(2),1])
            ...
            if (...
                ...
                warning('%s have less than %.0f%% of usable pixels, or less than minimul relative good pixels. We don''t skip it, but it might not be healthy.', ...
