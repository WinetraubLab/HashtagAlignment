...
patchSize_pix = 256; % Patch size (pixels)
...
for iSlide=1:length(isUsable)
    ...
    img_he = imPadToMeetPatchSize(img_he, patchSize_pix, 0);
    img_oct = imPadToMeetPatchSize(img_oct, patchSize_pix, 0);
    img_mask = imPadToMeetPatchSize(img_mask, patchSize_pix, 10); 
    
    % define h -height and w-width of images after downsize
    h=size(img_he,1); w=size(img_he,2);
    
    % Recolor to identify empty regions.
    img_he = imReColor(img_he,[0 0 0],[0 255 0]);
    img_oct = imReColor(img_oct,[0 0 0],[0 255 0]);
    
    %% Generate patches 
    if h<patchSize_pix || w<patchSize_pix
    ...
    cropcount = 1;
    for y=1:patchOverlap:(h-patchSize_pix)
        for x =1:patchOverlap:(w-patchSize_pix)
