...
% middle of the line.
%USAGE:
%   [ptsPixPosition, ptsId, ptsRes , lnLen] = findLines (img,lnNames)
%INPUTS:
...
%   ptsRes - Residual square error from least squares fitting
%   lnLen - The average length of each line
%  
%Example:
...
function [ptsPixPosition, ptsId, ptsRes , lnLen] = findLines (img,lnNames,y) 
    ...
    if y==1
        ...
        for i=1:length(lnNames)
            ...
            for y=round(linePts(1,2,i)):round(linePts(2,2,i)) %ystart to yend
                ...
                ssq = sum(((data-gaussian(a,x)).*(weight)).^2);
                ssq = ssq + 1e-10;
                % discard bad fits
    ...
    %Ask user to mark lines by their order
