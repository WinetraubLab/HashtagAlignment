%Collects Data for Simulation of Photobleaching and Alignment Algorithm
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Purpose:
%   - collects data in a loop by jittering the angle and position of lines
...
numdatapts = 50;
sampling = linspace(10,250,15);
data = zeros(length(sampling),numdatapts);

 %% GUI Hendling
img = flipud(double(img)/255 .* sample_mask);
%Draw image 
figure(21);
subplot(1,1,1);
imagesc(img);
...
for i = 1:length(sampling)
    ...
    for j = 1:numdatapts
        ...
        [ptsPixPosition, ptsId, lnLen] = findLines_collectdata(slice_masked_,lnNames, linePts, linePs, sampling(i));
        ...
        data(i,j) = abs(acos(dot(v/norm(v),[0;0;1]))*180/pi-acos(dot(v_out/norm(v_out),[0;0;1]))*180/pi);

    end
end

figure; errorbar(sampling,mean(data,2),std(data,0,2));
ylim([0 4])
